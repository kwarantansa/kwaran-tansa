...
interface GudepManagerProps {
  ...
  onDeleteRegistration?: (regId: string) => Promise<void> | void;
      ...
      {filteredRegistrations.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredRegistrations.map((reg) => {
            const duplicateCount = registrations.filter(r => 
              r.namaPangkalan.trim().toLowerCase() === reg.namaPangkalan.trim().toLowerCase() ||
              (r.akunGudep?.username && reg.akunGudep?.username && r.akunGudep.username.trim().toLowerCase() === reg.akunGudep.username.trim().toLowerCase())
            ).length;
            ...
            return (
              <div
                key={reg.id}
                className={`bg-white rounded-2xl border ${isDuplicate ? 'border-amber-300 ring-1 ring-amber-200' : 'border-[#E5DFD5]'} p-5 shadow-sm space-y-4 hover:border-amber-400 transition-all flex flex-col justify-between`}
              >
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {isDuplicate && (
                          <span className="text-[10px] font-bold bg-amber-100 text-amber-900 px-2 py-0.5 rounded-full border border-amber-300 flex items-center gap-1 shadow-2xs">
                            ⚠️ Terdaftar {duplicateCount}x (Duplikat)
                          </span>
                        )}
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {onDeleteRegistration && (
                        <button
                          onClick={() => {
                            if (confirm(`Hapus berkas pendaftaran ini?\n\n• Pangkalan: ${reg.namaPangkalan}\n• No. Registrasi: ${reg.noRegistrasi}\n• Status: ${reg.statusVerifikasi}\n\nData pendaftaran yang dihapus tidak dapat dipulihkan.`)) {
                              onDeleteRegistration(reg.id);
                            }
                          }}
                          className="p-1.5 text-stone-400 hover:text-red-700 hover:bg-red-50 rounded-lg border border-transparent hover:border-red-200 transition-colors"
                          title="Hapus Berkas Pendaftaran (Duplikat / Batal)"
                        >
                  ...
                  <div className="flex items-center gap-2">
                    {onDeleteRegistration && (
                      <button
                        ...
                        className="px-2.5 py-1.5 bg-red-50 hover:bg-red-100 text-red-700 hover:text-red-800 font-semibold text-xs rounded-xl border border-red-200 flex items-center gap-1 transition-colors"
                        title="Hapus berkas pendaftaran (duplikat atau batal)"
                      >
                        <Trash2 className="w-3.5 h-3.5" />
                        <span>Hapus</span>
                      </button>
